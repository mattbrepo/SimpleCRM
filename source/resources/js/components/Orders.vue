  ...
  <div class="normal-center">
    ...
    <b-table striped hover 
      ...
      <template #cell(edit)="row">
        <button v-if="!isAdmin" style="margin-left: 1.5em;" class="btn-dark" @click="editRow(row.item)">View</button>
        <button v-if="isAdmin" style="margin-left: 1.5em;" class="btn-dark" @click="editRow(row.item)">Edit</button>
  ...
  methods: {
    fetchOrders() {      
      axios.get('/api/orders').then(response => {
        ...
        this.$alert('Error fetching orders', '', 'error');
      });
    },
    onRowSelected(items) {
      this.selected = items
    ...
    deleteRow(item) {
      this.$confirm('Are you sure you want to delete this order?', '', 'warning').then(() => {      
        axios.delete('/api/order/' + item.id).then(response => {
          this.orders = response.data;
          this.$forceUpdate(); // necessary to recompute filtered()
        }).catch((ex) => {
          var msg = ex.response.data.message || 'Error deleting order';
    ...
    getGroupAndCheck() {
      ...
      if (this.selected.length <= 0) {
        this.$alert('No order selected', '', 'warning');
        return -1;
      }
    ...
    assignGroup() {
      ...
      this.$confirm('Are you sure you want to assign these orders to the selected group?', '', 'warning').then(() => {      
        var post_pars = {
          orders: this.selected
        };
        axios.post('/api/group/' + group_id + '/assign-orders', post_pars).then(response => {
          this.$alert('Companies assigned', '', 'success');
        }).catch((ex) => {
          var msg = ex.response.data.message || 'Error assigning order to grouè';
          this.$alert(msg, '', 'error');
        });
      });
    },
    removeGroup() {
      ...
      this.$confirm('Are you sure you want to remove these orders from the selected group?', '', 'warning').then(() => {      
        var post_pars = {
          orders: this.selected
        };
        axios.post('/api/group/' + group_id + '/remove-orders', post_pars).then(response => {
          this.$alert('Companies removed', '', 'success');
        }).catch((ex) => {
          var msg = ex.response.data.message || 'Error removing order from group';
          this.$alert(msg, '', 'error');
        });
      });
    }
